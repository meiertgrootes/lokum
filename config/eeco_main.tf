...
data "template_file" "eeco-template" {
        template = "${file("eecolidargen2-opennebula_k8s.tpl")}"
}

resource "opennebula_template" "eeco-template" {
        name = "eeco-template"
        description = "${data.template_file.eeco-template.rendered}"
        permissions = "600"
}

resource "opennebula_vm" "eeco-node" {
        name = "eeco${count.index+1}"
        template_id = "${opennebula_template.eeco-template.id}"
        permissions = "600"
        count = "${var.number_of_nodes}"
}

resource "null_resource" "eecocluster" {

        provisioner "local-exec" {
                command = "/bin/bash -c \"declare -a IPS=(${join(" ", opennebula_vm.eeco-node.*.ip)})\""
        }

        provisioner "local-exec" {
                command = "CONFIG_FILE=${var.DEPLOY_FOLDER}/hosts.yaml python3 /lokum/inventory_builder/inventory.py ${join(" ", opennebula_vm.eeco-node.*.ip)}"
        }
...
output "eeco-node-vm_id" {
        value = "${opennebula_vm.eeco-node.*.id}"
}

output "eeco-node-vm_ip" {
        value = "${opennebula_vm.eeco-node.*.ip}"
}
